...
async def get_processes():
    async with db_service.get_session() as session:
        ...
        result = await session(statement)
        processes = result.all()

        return [{"id": p.id, "name": p.name_process} for p in processes]
    ...
    async with db_service.get_session() as session:
        ...
        if process_data.candidate_ids:
            for cid in process_data.candidate_ids:
                ...
                if not candidate:
                    raise HTTPException(
                        status_code=404, detail=f"Candidate id {cid} not found"
                    )

                link = ProcessCandidateLink(process_id=process.id, candidate_id=cid)
                session.add(link)
